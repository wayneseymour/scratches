...
export async function pickTestGroupRunOrder() {
  ...
  bk.uploadSteps(
    [
      ...
      browserTests(functionalGroups, defaultQueue, FTR_CONFIGS_DEPS, FTR_CONFIGS_RETRY_COUNT),
    ].flat()
  );
}

function unitTests(x: any) {
  const res =
    x.count > 0
      ? {
          label: 'Jest Tests',
...
function integrationTests(x: any) {
  const res =
    x.count > 0
      ? {
...
function browserTests(x: any, defaultQueue: string, ftrConfigDeps: any, ftrConfigsRetryCount: any) {
  const res = x.length
    ? {
        group: 'FTR Configs',
        key: 'ftr-configs',
        depends_on: ftrConfigDeps,
        steps: x
          .sort((a, b) =>
            ...
            ({ title, key, queue = defaultQueue }): BuildkiteStep => ({
              ...
              retry: {
                automatic: [
                  ...
                  ...(ftrConfigsRetryCount > 0
                    ? [{ exit_status: '*', limit: ftrConfigsRetryCount }]
                    : []),
                ],
              },
            })
